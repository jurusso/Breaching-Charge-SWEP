	...
	if ( self.Weapon:Clip1() > 0 ) then 
		...
		if ( tr.Entity:GetClass( ) == "prop_door_rotating" && tr.Entity:GetPos( ):Distance( self.Owner:GetPos( ) ) < 100 ) and tr.Entity.hascharge == nil then
			...
			ent:Spawn( ) 
				...
				timer.Simple( .25, function( )if IsValid(ent) then ent:EmitSound( "ui/beep07.wav" ) end end )
				timer.Simple( .5, function( )if IsValid(ent) then ent:EmitSound( "ui/beep07.wav" ) end end ) 
				timer.Simple( .75, function( )if IsValid(ent) then ent:EmitSound( "ui/beep07.wav" ) end end )
